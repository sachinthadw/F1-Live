...
import { ChampionshipEntry, RaceResult } from '../types';
...
export async function getRaceResults(year: number, round: number): Promise<RaceResult[]> {
    try {
        ...
        return results.map((r: any) => ({
            ...
            team_colour: getTeamColor(r.Constructor.name),
            acronym: r.Driver.code || r.Driver.familyName.substring(0,3).toUpperCase()
        }));
    } catch (e) {
